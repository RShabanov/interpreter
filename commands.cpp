...
void Cmd::assign_var() {
	...
	if (token_type == DELIMITER) {
		...
		if (is_defined) create_variable(&var_name[0], var_value);
...
void Cmd::define_var(double& _value, bool& is_defined) {
	if (tok != EOL && tok != FINISHED) {
		...
		/*else if (token[0] != ',') 
			throw VariableException(NOT_INITIALIZED);*/
		else throw ParserException(INVALID_SYNTAX);
	}
}
...
void Cmd::cmd_def() {}
void Cmd::cmd_let() {
	// изначальная строка, например: let var = 5
